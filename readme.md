...
| KELPIE_API_REQUEST_TIMEOUT_S    | Maximum duration of one Kelpie API HTTP attempt.                                                                                                                                                                                                                                                           | "10"                              |
| KELPIE_API_RETRY_INITIAL_DELAY_S | Delay after the first failed Kelpie API attempt. Later delays increase exponentially.                                                                                                                                                                                                                     | "5"                               |
| KELPIE_API_RETRY_MAX_DELAY_S    | Maximum delay between Kelpie API attempts.                                                                                                                                                                                                                                                                 | "30"                              |
| KELPIE_API_URL                  | The URL for the Kelpie API                                                                                                                                                                                                                                                                                 | `https://kelpie.saladexamples.com` |
| KELPIE_CANCEL_PROGRESS_LOG_INTERVAL_S | How often to log that Kelpie is still waiting for a remotely canceled job to exit. Set to `0` to disable progress logs.                                                                                                                                                                             | "10"                               |
| KELPIE_CANCEL_SIGINT_GRACE_S    | Seconds to wait after sending `SIGINT` to a canceled job's process group before escalating.                                                                                                                                                                                                                | "15"                               |
| KELPIE_CANCEL_SIGTERM_GRACE_S   | Seconds to wait after sending `SIGTERM` to a canceled job's process group before escalating.                                                                                                                                                                                                               | "15"                               |
| KELPIE_CANCEL_SIGKILL_GRACE_S   | Seconds to wait after sending `SIGKILL` to a canceled job's process group before requesting recreate or continuing cleanup.                                                                                                                                                                               | "3"                                |
| KELPIE_LOG_LEVEL                | The log level for kelpie                                                                                                                                                                                                                                                                                   | "info"                             |
| KELPIE_JOB_HEARTBEAT_INTERVAL_S | How often the worker sends a heartbeat while a job is running. The job's submitted `heartbeat_interval` still controls server-side stale-assignment timing.                                                                                                                                                | "30"                              |
| KELPIE_RECREATE_AFTER_CANCELED_JOB | Whether to recreate the container after any remotely canceled job. When enabled, Kelpie still first makes a bounded local attempt to stop the job process group.                                                                                                                                        | "false"                            |
...
| KELPIE_RECREATE_EVERY_N_JOBS    | Recreate the environment after this many finalized jobs. Set to `0` to disable. If `KELPIE_RECREATE_BETWEEN_JOBS=true`, that takes precedence and recreates after every job.                                                                                                                              | "0"                                |
...
| KELPIE_WORK_POLL_INTERVAL_S     | Delay between idle `/work` polls and after an exhausted `/work` operation. Takes precedence over legacy `HEARTBEAT_INTERVAL_S`.                                                                                                                                                                           | "30"                              |
| HEARTBEAT_INTERVAL_S            | Legacy fallback for the `/work` polling interval. It does not control job heartbeats. Prefer `KELPIE_WORK_POLL_INTERVAL_S`.                                                                                                                                                                                | "30"                              |
| MAX_JOB_FAILURES                | The maximum number of job failures allowed before an instance should reallocate.                                                                                                                                                                                                                           | "3"                                |
| MAX_RETRIES                     | The maximum number of attempts for an API operation, including the first attempt.                                                                                                                                                                                                                          | "3"                                |
| MAX_TIME_WITH_NO_WORK_S         | The maximum time to wait for work before exiting. May be exceeded by up to one work-poll interval.                                                                                                                                                                                                         | "0" (Never)                        |
...
| SALAD_IMDS_URL                  | The Salad IMDS base URL. This is primarily useful for local tests; production should use the default metadata service.                                                                                                                                                                                     | `http://169.254.169.254`           |
